...
Flask + Flask-Login + SQLite + Tailwind CSS を使用した学習プラットフォーム
"""
    ...
    if not os.path.exists(app.config['DATABASE']):
        ...
        try:
            ...
            with get_db_connection(app.config['DATABASE']) as conn:
                ...
                print("ユーザーテーブルとデフォルト管理者を追加しました")
    ...
    try:
        ...
        for filename in os.listdir(app.config['JSON_FOLDER']):
            if filename.endswith('.json'):
                try:
                    with open(os.path.join(app.config['JSON_FOLDER'], filename), 'r', encoding='utf-8') as f:
                        questions = json.load(f)
                    saved_count = question_manager.save_questions(questions)
                    total_questions += saved_count
                except Exception as e:
                    print(f"ファイル {filename} 読み込みエラー: {e}")
        
        if total_questions > 0:
            print(f"JSONフォルダから {total_questions}問を読み込みました。")
    except Exception as e:
        print(f"JSON自動読み込みエラー: {e}")
...
def parse_filename_info(filename):
    ...
    if match:
        ...
        return {'year': year, 'season': season, 'season_jp': season_jp, 'display_name': f'{year}年{season_jp}'}
    return None
        ...
        if os.path.exists(app.config['JSON_FOLDER']):
            for filename in os.listdir(app.config['JSON_FOLDER']):
                if filename.endswith('.json'):
                    ...
                    if file_info:
                        json_files.append({'filename': filename, 'info': file_info})
