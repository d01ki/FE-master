...
# FE-Master シンプルセットアップスクリプト

set -e

echo "🔧 FE-Master プロジェクト構造を整理中..."
echo "=============================="

# バックエンドフォルダから必要なファイルをルートに移動
if [ -d "backend" ]; then
    echo "📁 backend フォルダからファイルを移動中..."
    
    # requirements.txt を移動
    if [ -f "backend/requirements.txt" ]; then
        cp backend/requirements.txt ./requirements.txt
        echo "✅ requirements.txt を移動しました"
    fi
    
    # 有用なスクリプトがあれば移動
    if [ -d "backend/scripts" ]; then
        cp -r backend/scripts ./scripts_backup
        echo "✅ scripts フォルダをバックアップしました"
    fi
    
    # backendフォルダをバックアップ
    mv backend backend_backup
    echo "✅ backend フォルダをバックアップしました"
fi

# 簡単なrequirements.txtを作成（存在しない場合）
